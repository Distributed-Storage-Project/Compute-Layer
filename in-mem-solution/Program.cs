...
class Program
{
    static void Main(string[] args)
    {
        ...
        */
        ...
        var results = logDatabase.QueryLogs(
    timestamp,
    timestamp2,
    filterMap.ContainsKey("Level") ? filterMap["Level"] : null,
    filterMap.ContainsKey("Service") ? filterMap["Service"] : null,
    limit);
...
public class InMemoryLogDatabase
{
    ...
    int? limit = null)
    {
        ...
        {
            logs = logs.Where(log => levels.Contains(log.Level));
        }

        if (services != null && services.Count > 0)
        {
            logs = logs.Where(log => services.Contains(log.Service));
        }
